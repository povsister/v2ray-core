...
type Area struct {
	...
	pendingRemoveMaturedLSAs   map[packet.LSAIdentity]struct{}
	...
	pendingWrappingLSAs map[packet.LSAIdentity]packet.LSAdvertisement
...
func (a *Area) shutdown() {
	...
	for _, ifi := range a.Interfaces {
		if err := ifi.close(); err != nil {
			logErr(err, "interface %v close failed", ifi.c.ifi.Name)
		}
	}
	a.wg.Wait()
}
